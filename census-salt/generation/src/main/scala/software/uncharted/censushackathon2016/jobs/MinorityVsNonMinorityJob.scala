...
object MinorityVsNonMinorityJob {
  def run(data: DataFrame, levelBatches: Seq[Seq[Int]], outputterFactory: OutputterFactory) = {
    ...
    val input = Pipe(data)
      .to(_.select("longitude", "latitude", "applicantRace", "applicantRaceProb"))
      ...
      .to(sourceData => {
        ...
        .to(_.map(level => {
          ...
          val nonMinority = new layers.CultureHeatMapLayer("non-minority-permits", level, (c: String) => c == "white")
          val minorities = new layers.CultureHeatMapLayer("minority-permits", level, (c: String) => c != "white")
          ...
          val tiles = gen.generate(
            sourceData,
            Seq(nonMinority.series, minorities.series),
            request
          )
          nonMinority.serialize(level, tiles, outputterFactory)
          minorities.serialize(level, tiles, outputterFactory)
        }))
        .run;
