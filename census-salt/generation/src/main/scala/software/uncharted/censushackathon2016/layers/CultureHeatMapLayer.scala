...
class CultureHeatMapLayer(layerName: String, levels: Seq[Int], cultureFilter: (String) => Boolean) extends Serializable {
  private val tileSize = 256;

  // Given an input row, return permit longitude, latitude as a tuple
  private val permitExtractor = (r: Row) => {
    if (r.isNullAt(0) || r.isNullAt(1) || r.isNullAt(2) || r.isNullAt(3) || !cultureFilter(r.getString(2))) {
      None
    } else {
